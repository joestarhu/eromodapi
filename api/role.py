...
def get_list(*,user=Depends(get_user),page=Depends(get_page),name:str=Query(default='',description='角色名'),status:int=Query(default=None,description='角色状态')):
    data = RoleList(**page,name=name,status=status)
    return role_api.get_list(user['db'],user['act'],data)
